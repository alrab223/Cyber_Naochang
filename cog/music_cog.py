    ...
    async def on_voice_state_update(self,member,before,after):
        ...
        with (sqlite3.connect("db/bot_data.db")) as conn:
            ...
            if after.channel is None and len(before.channel.members) == 1:
                c.execute(f'update vc_notification_setting set reset=0')
            try:
                sql = c.execute(f'select id,members,reset,vc_notification from vc_notification_setting where vc_notification=1')
                id_list = [x for x in sql]
                for i in id_list:
                    if len(after.channel.members)>=i[1] and i[3]==1 and i[2]==0:
                        dm_channel = self.bot.get_user(i[0])
                        await dm_channel.send(f"{after.channel.name}に{len(after.channel.members)}以上います")
                        c.execute(f'update vc_notification_setting set reset=1 where id={i[0]}')      
        ...
        if before.channel is None and self.voich!=None:
            if random.randint(1, 10) <4:
               ...
               self.voich.play(discord.FFmpegPCMAudio(random.choice(text).replace("\n", "")), after=check_error)
               self.voich.source=discord.PCMVolumeTransformer(self.voich.source)
               self.voich.source.volume=self.volume
            else:
