...
class Main(commands.Cog):
   ...
   async def sql_changer(self, ctx):
      ...
      for data in datas:
         self.db.insert(
             "vc_notification", [
                 "id", "members", "reset", "cokumn", "vc_notification"], list(data))
      print("完了")
   ...
   async def member_join(self, ctx):
      ...
      for member in members:
         if self.db.select(
            f"select* from user_join where id={member.id}") is not None:
            times = member.joined_at + datetime.timedelta(hours=9)
            self.db.allinsert(
                "user_join", [
                    member.id, times.year, times.month, times.day])
   ...
   async def future_send(self, ctx, time: str, channel_id: int):
      ...
      self.db.allinsert(
          "future_send", [
              ctx.author.id, msg.content, time, channel_id])
   ...
   async def member(self, ctx):
      ...
      for member in guild.members:
         if member.bot or self.db.select(
            f"select* from userdata where id={member.id}") is not None:
            pass
         else:
            self.db.insert(
                "user_data", [
   ...
   async def on_reaction_add(self, reaction, user):
      ...
      if reaction.count == 1:
         if reaction.emoji == "🇺🇸":
            ...
            trans_en = translator.translate(
                reaction.message.content, src='ja', dest='en')
            await reaction.message.channel.send(trans_en.text)
         elif reaction.emoji == "🇯🇵":
            translator = Translator()
            lang = translator.detect(reaction.message.content)
            trans_en = translator.translate(
                reaction.message.content, src=lang.lang, dest='ja')
            await reaction.message.channel.send(trans_en.text)
         elif reaction.emoji == "🇮🇹":
            translator = Translator()
            trans_en = translator.translate(
                reaction.message.content, src='ja', dest='it')
            await reaction.message.channel.send(trans_en.text)
   ...
   async def on_message(self, message):
      ...
      if message.channel.id == channels[0].id or message.channel.id == channels[
         1].id or message.channel.id == channels[2].id or message.channel.id == channels[3].id:
